...
models.load_models([
    ...
    },
    {
        model: 'product.pack',
        condition: function(self){ return true; },
        fields: ['id','product_name','product_quantity','uom_id','price','wk_product_template'],
...
models.Orderline = models.Orderline.extend({
    getPackProduct:function(pack_product_id,product_price,product_qty){
        ...
        for(var i=0;i<pack_product.length;i++)
        {   
            if(pack_product[i].id==pack_product_id && self.product.is_pack && (pack_product[i].wk_product_pack).length>0 )
            {
                for(var j=0;j<(pack_product[i].wk_product_pack).length;j++){ 
                    ...
                    for(var k=0;k<wk_pack_products.length;k++){
                        ...
                        var product_val={'display_name':wk_pack_products[k].product_name[1],'uom_id':wk_pack_products[k].uom_id,'price':wk_pack_products[k].price};
